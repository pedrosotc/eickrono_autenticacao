...
DIAS_VALIDOS=${1:-825}
SENHA_KEYSTORE=${MTLS_KEYSTORE_SENHA:-senhaBackchannelDev}
SENHA_TRUSTSTORE=${MTLS_TRUSTSTORE_SENHA:-senhaBackchannelDev}
...
openssl req -x509 -newkey rsa:4096 \
  ...
  -nodes \
  -sha256 \
  -subj "/CN=eickrono-backchannel-dev-ca/O=Eickrono/OU=Desenvolvimento"
...
gerar_certificado \
  ...
  "eickrono-autenticacao" \
  "DNS:eickrono-autenticacao,DNS:host.docker.internal,DNS:localhost,IP:127.0.0.1" \
...
gerar_certificado \
  "eickrono-keycloak" \
  "eickrono-keycloak" \
  "DNS:eickrono-keycloak,DNS:host.docker.internal,DNS:localhost,IP:127.0.0.1" \
  "clientAuth"
...
echo "Exemplos:"
echo "  cd /Users/thiago/Desenvolvedor/flutter/eickrono-autenticacao-servidor/infraestrutura/dev && docker compose up -d --force-recreate eickrono-autenticacao identidade-servidor eickrono-keycloak"
echo "  cd /Users/thiago/Desenvolvedor/flutter/eickrono-thimisu-backend/infraestrutura/dev && docker compose up -d --force-recreate thimisu-backend"
