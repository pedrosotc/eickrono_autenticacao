...
@Component
public final class ClienteTokenBackchannelIdentidadeKeycloak {
    ...
    public ClienteTokenBackchannelIdentidadeKeycloak(final RestTemplateBuilder restTemplateBuilder,
                                                     final IdentidadeBackchannelProperties properties) {
        IdentidadeBackchannelProperties.JwtInterno jwtInterno = Objects.requireNonNull(
        ...
        this.urlToken = normalizarUrlBase(jwtInterno.getUrlBase())
                + "/realms/" + obrigatorio(jwtInterno.getRealm(), "integracao.identidade.jwt-interno.realm")
                + "/protocol/openid-connect/token";
        this.clientId = obrigatorio(jwtInterno.getClientId(), "integracao.identidade.jwt-interno.client-id");
        this.clientSecret = obrigatorio(jwtInterno.getClientSecret(), "integracao.identidade.jwt-interno.client-secret");
    }
    ...
    public String obterTokenBearer() {
        ...
        synchronized (this) {
            ...
            if (payload == null || !payload.hasNonNull("access_token")) {
                throw new IllegalStateException("Falha ao obter access token interno para o backchannel de identidade.");
            }
    ...
    private static String normalizarUrlBase(final String urlBase) {
        String valor = obrigatorio(urlBase, "integracao.identidade.jwt-interno.url-base");
